    ...
    <div className="bg-gray-0 shadow-card flex items-center justify-between gap-4 rounded-(--radius-md) p-4">
      <div className="flex flex-col gap-1.5">
        <div className="flex items-center gap-2">
          <h3 className="subhead2 text-gray-400">{name}</h3>
          {operationStatus && (
            <Chip
        ...
        {address && (
          <div className="flex items-center gap-1">
            ...
            <span className="body2 text-gray-300">{address}</span>
          </div>
        )}
        {todayHours && (
          <div className="flex items-center gap-1">
            <IcClock size={14} className="shrink-0 text-gray-300" />
            <span className="body2 text-gray-300">{todayHours}</span>
          </div>
        )}
        {mapUrl && (
          <a
            ...
            className="body2 text-primary mt-0.5 flex w-fit items-center gap-0.5 underline-offset-2 hover:underline"
          >
            지도 보기
          </a>
        )}
